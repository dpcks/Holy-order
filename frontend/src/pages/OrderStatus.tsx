  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      if (event.data === 'ORDER_UPDATED') {
        console.log('User side: Real-time update received');
        fetchData(false);
      }
    };

    // 폴링도 백업으로 유지 (교회 Wi-Fi 불안정 대비)
    ...
    <div className="flex flex-col min-h-screen w-full max-w-[500px] mx-auto bg-[#F9FAFB] pb-8 shadow-2xl relative">
      ...
      <main className="flex-1 px-4 py-4 flex flex-col items-center gap-5">
            ...
            <div className="px-5 py-5 flex flex-col gap-4">
              <div className="text-center">
                ...
                <p className="text-[42px] font-black text-primary tracking-tight leading-none">
                  {totalAmount.toLocaleString()}<span className="text-[22px] font-bold ml-1">원</span>
                </p>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 mt-1">
                  <span className="text-[16px] font-black text-[#2D1616] tracking-wide">{setting.account_number}</span>
                  <button onClick={handleCopyAccount} className={`flex items-center gap-1.5 text-[12px] font-bold px-3 py-1.5 rounded-lg transition-all ${copied ? 'bg-green-100 text-green-600' : 'bg-primary/10 text-primary hover:bg-primary/20'}`}>
        ...
        <div className="text-center py-2">
          ...
          <div className="inline-flex items-center gap-2 bg-white border border-gray-100 shadow-sm px-5 py-2.5 rounded-full">
            <div className={`w-2.5 h-2.5 rounded-full animate-pulse ${isReady || isCompleted ? 'bg-green-500' : isPreparing ? 'bg-primary' : 'bg-orange-400'}`} />
        ...
        <div className="w-full max-w-[340px] flex justify-between items-center relative px-2 py-4">
          ...
          <div className="absolute top-[34px] left-10 h-1 bg-primary -z-10 transition-all duration-1000 ease-in-out rounded-full" style={{ width: (isReady || isCompleted) ? 'calc(100% - 5rem)' : isPreparing ? 'calc(50% - 2.5rem)' : '0%' }} />
          <div className="flex flex-col items-center gap-2.5">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center z-10 transition-all duration-500 ${!isPending ? 'bg-[#2D1616] text-white shadow-lg' : 'bg-white border-2 border-gray-200 text-gray-300'}`}><CheckCircle2 size={24} /></div>
            <span className={`text-[12px] font-bold ${!isPending ? 'text-[#2D1616]' : 'text-gray-400'}`}>입금 확인</span>
          </div>
          <div className="flex flex-col items-center gap-2.5">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center z-10 transition-all duration-500 ${isPreparing ? 'bg-primary text-white shadow-[0_0_20px_rgba(255,75,75,0.4)] scale-110' : (isReady || isCompleted) ? 'bg-[#2D1616] text-white shadow-lg' : 'bg-white border-2 border-gray-200 text-gray-300'}`}><Coffee size={24} /></div>
            <span className={`text-[12px] font-bold ${isPreparing ? 'text-primary' : (isReady || isCompleted) ? 'text-[#2D1616]' : 'text-gray-400'}`}>준비 중</span>
          </div>
          <div className="flex flex-col items-center gap-2.5">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center z-10 transition-all duration-500 ${isReady ? 'bg-primary text-white shadow-[0_0_20px_rgba(255,75,75,0.4)] scale-110' : isCompleted ? 'bg-[#2D1616] text-white shadow-lg' : 'bg-white border-2 border-gray-200 text-gray-300'}`}><PartyPopper size={24} /></div>
        ...
        <div className="w-full bg-white rounded-2xl p-6 shadow-sm border border-gray-50 mt-2">
          <h4 className="font-black text-gray-900 text-[15px] tracking-tight mb-5 pb-4 border-b border-gray-50">주문 내역 요약</h4>
          {order?.items?.map((item: any, idx: number) => (
            <div key={idx} className="flex gap-4 items-center mb-4 last:mb-0">
              <div className="w-14 h-14 rounded-xl overflow-hidden bg-gray-100 shrink-0 border border-gray-50"><img src="https://images.unsplash.com/photo-1559525839-b184a4d698c7?w=100&q=80" alt="coffee" className="w-full h-full object-cover opacity-90" /></div>
              <div className="flex-1 min-w-0">
                <h5 className="font-bold text-[#2D1616] text-[15px] truncate mb-0.5">{item.menu_name_snapshot}</h5>
                <p className="text-gray-400 text-[12px] font-semibold">수량: {item.quantity}개 {item.options_text ? `· ${item.options_text}` : ''}</p>
              </div>
              <span className="text-[14px] font-black text-gray-800">₩{item.sub_total.toLocaleString()}</span>
            </div>
          ))}
          <div className="mt-6 pt-5 border-t border-gray-100 flex justify-between items-center">
            <span className="text-gray-400 text-[13px] font-bold uppercase tracking-wider">Total Amount</span>
            <span className="font-black text-[20px] text-primary">₩{totalAmount.toLocaleString()}</span>
          </div>
        </div>

        <div className="w-full bg-gray-100/50 rounded-2xl p-5 flex gap-4 items-start border border-gray-200/50">
          <div className="bg-white p-2 rounded-xl shadow-sm shrink-0 border border-gray-100"><Coffee className="text-primary" size={22} /></div>
          <div>
            <h5 className="font-bold text-gray-900 text-[14px] mb-1">실시간 연결됨</h5>
            <p className="text-gray-500 text-[12px] leading-relaxed font-medium">관리자가 상태를 변경하면 <span className="text-primary font-bold text-[13px]">즉시</span> 반영됩니다. 화면을 유지해 주세요.</p>
          </div>
        </div>
      </main>
    </div>
